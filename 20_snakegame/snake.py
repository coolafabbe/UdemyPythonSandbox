from turtle import Turtle

STARTING_POSITIONS = [(0,0), (-20,0), (-40,0)]

MOVE_DISTANCE = 20
COLLISION_DISTANCE = 15
UP = 90
DOWN = 270
LEFT = 180
RIGHT = 0

class Snake():
    def __init__(self):
        self.segments = []
        self.create_snake()
        self.head = self.segments[0]
        
    def create_snake(self):
        for position in STARTING_POSITIONS:
            self.create_segment(position)

    def move(self):
        for seg_num in range(len(self.segments)-1, 0, -1):
            x = self.segments[seg_num-1].xcor()
            y = self.segments[seg_num-1].ycor()
            if (x, y) != self.segments[seg_num].position:
                self.segments[seg_num].goto(x,y)
            
        self.head.forward(MOVE_DISTANCE)

    def up(self):
        if self.head.heading() != DOWN:
            self.head.setheading(UP)

    def down(self):
        if self.head.heading() != UP:
            self.head.setheading(DOWN)

    def left(self):
        if self.head.heading() != RIGHT:
            self.head.setheading(LEFT)

    def right(self):
        if self.head.heading() != LEFT:
            self.head.setheading(RIGHT)

    def create_segment(self, position):
        new_snake_segment = Turtle(shape="square")
        new_snake_segment.penup()
        new_snake_segment.color("white")   

        if position == "new":
            new_snake_segment.goto(self.segments[-1].position()) 
        else: 
            new_snake_segment.goto(position)

        self.segments.append(new_snake_segment)

    def check_collision(self, collided, check):
        if check == "all":
            for seg in self.segments:
                if seg != collided:
                    if seg.distance(collided) < COLLISION_DISTANCE:
                        return True
        elif check == "head":
            if self.head.distance(collided) < COLLISION_DISTANCE:
                return True
        return False

    def check_walls(self):
        if self.head.xcor() < -290 or self.head.xcor() > 290 or self.head.ycor() < -290 or self.head.ycor() > 290:
            return True
        else:
            return False